...
root_path = "F:\\Animal_behavior_analysis\\Dataset\\video_dataset\\03"
save_root_path = "F:\\Animal_behavior_analysis\\Dataset\\img_dataset"
save_path=save_root_path+"\\"+"JPEGImages"+"\\"

# obj_id、obj_name、i全局变量，不要动！！！！
obj_id=[]
obj_name=[]
...
def genXML(xml_dir, outname, bboxes, width, height):
    ...
    xml_file.write('    <folder>VOCdataset</folder>\n')
    xml_file.write('    <path>%s</path>\n'%(save_path+outname+'.jpg'))
    ...
    for bbox in bboxes:
        ...
        xml_file.write('        <name>' + obj_name[i]+str(obj_id[i]) + '</name>\n')
...
def assign_jpgAndAnnot(src_annot_file, dst_annot_dir, dst_jpg_dir, tmp_video_frame_save_dir, width, height):
    ...
    for line in content:
        item = line[:-1]
        items = item.split(',')
        frame_id, num_of_object = items[0], items[1]
        print_flush("Assign jpg and annotion : %s" % frame_id)

        bboxes = []

        for i in range(int(num_of_object)):
            obj_id.append(items[1 + i * 6 + 1])
            obj_name.append(items[1 + i * 6 + 6])
        ...
        genXML(dst_annot_dir, append_name + "_" + str(frame_id), bboxes, width, height)
        shutil.copy(
...
if __name__ == "__main__":
    append_names = ["0%d" % i for i in range(0, 10)]
    ...
    for append_name in append_names:
        ...
        dst_annot_dir = os.path.join(save_root_path, "Annotations")
        dst_jpg_dir = os.path.join(save_root_path, "JPEGImages")
