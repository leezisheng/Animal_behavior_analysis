...
# =============================================全局变量=========================================================
# 测试txt文件路径
test_txt_path                          = "F:\\Animal_behavior_analysis\\output\\results.txt"
# 结果图片保存地址
mask_result_img_save_path              = 'F:\\Animal_behavior_analysis\\Data_processing\\result\\center_move_in_mask'
single_object_motion_plot_save_path    = 'F:\\Animal_behavior_analysis\\Data_processing\\result\\single_object_motion_plot'
multi_object_conv_histogram_save_path  = 'F:\\Animal_behavior_analysis\\Data_processing\\result\\motion_information_Histogram'
# 目标数量
object_num = 3
...
# 数据txt文件保存名字
save_txt_name = 'object_move_info.txt'
# 数据txt文件保存地址
save_path     = 'F:\\Animal_behavior_analysis\\Data_processing\\result'+'\\'+ save_txt_name
...
for object_id in range(1,object_num+1):
    ...
    try:
        ...
        # 选出id号代表的某object的运动轨迹数据列表
        move_data_list = Get_move_data(line_data_list, object_id)
        print('解析文件中数据成功')
    except:
        print('解析文件中数据失败')
    ...
    print('序号为：' + str(object_id) + '的对象总移动距离为：' + str(total_distance) + '(m)')
    ...
    print('序号为：' + str(object_id) + '的对象总移动速度为：' + str(average_speed) + '(m/s)')
    ...
    print('序号为：' + str(object_id) + '的对象总移动加速度为：' + str(average_acc) + '(m/s2)')

    # 计算最大速度
    max_speed = Get_MaxValue(move_speed_list)
    print('序号为：' + str(object_id) + '的对象最大移动速度为：' + str(max_speed) + '(m/s)')

    # 计算最小速度
    max_acc   = Get_MaxValue(move_acc_list)
    print('序号为：' + str(object_id) + '的对象最大移动加速度为：' + str(max_acc) + '(m/s2)')

    # ============================================数据展示=======================================================

    # 显示固定id对象的速度-时间折线图
    Show_Speed_Plot(average_speed_list=move_speed_list,
                    img_save_path=single_object_motion_plot_save_path,
                    obj_id=object_id)
...
Show_Motion_Conv(average_motioninfo_list= average_distance_list,
                 img_save_path = multi_object_conv_histogram_save_path,
                 info_type = 'distance'
                 )
# 速度
...
object_mov_info_list = []

for i in range(1,object_num):
    # 对象运动数据列表
    temp_object_mov_info_list = []

    # 添加对象序号
    temp_object_mov_info_list.append(i)
    # 添加运动平均距离
    temp_object_mov_info_list.append(average_distance_list[i])
    # 添加运动平均速度
    temp_object_mov_info_list.append(average_speed_list[i])
    # 添加运动平均加速度
    temp_object_mov_info_list.append(average_acc_list[i])
    # 添加运动最大速度
    temp_object_mov_info_list.append(max_speed_list[i])
    # 添加运动最大加速度
    temp_object_mov_info_list.append(max_acc_list[i])

    object_mov_info_list.append(temp_object_mov_info_list)

print(object_mov_info_list)

# 写入txt文件
f = open(save_path, 'a')

for i in range(len(object_mov_info_list)):
    for j in range(len(object_mov_info_list[i])):
        # write函数不能写int类型的参数，所以使用str()转化
        f.write(str(object_mov_info_list[i][j]))
        # 相当于Tab一下，换一个单元格
        f.write('\t')
        # 写完一行立马换行
    f.write('\n')
